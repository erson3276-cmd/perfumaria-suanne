  ...
  if (lines.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-5 py-28 text-center">
        <p className="font-serif text-3xl text-ink">Seu carrinho está vazio</p>
    ...
    <div className="mx-auto max-w-7xl px-5 py-14 lg:px-8">
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_360px]">
        <div>
            ...
            {lines.map(({ product, qty }) => (
              <li key={product.slug} className="flex flex-col gap-4 py-6 sm:flex-row">
                <Link
                  ...
                  className="relative block h-36 w-28 shrink-0 overflow-hidden bg-white ring-1 ring-ink/5"
                >
                  <ProductImage
                    src={product.image}
                    alt={product.name}
                ...
                <div className="flex flex-1 flex-col justify-between">
                  ...
                  <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center border border-gold/40">
                      ...
                      <span className="w-9 text-center font-serif text-ink">
                        {qty}
                      </span>
                      <button
                        onClick={() => setQty(product.slug, qty + 1)}
          ...
          <button
            onClick={clear}
            className="mt-4 text-xs uppercase tracking-[0.15em] text-ink-soft underline-offset-4 hover:text-wine hover:underline"
          >
            Limpar carrinho
          </button>
        </div>
        ...
        <aside className="h-fit border border-gold/30 bg-cream p-7">
          ...
          <dl className="mt-5 space-y-3 text-sm">
            ...
            <div className="flex justify-between border-t border-gold/25 pt-3 text-ink">
          ...
          <Link
            href="/produtos"
            className="mt-3 block text-center text-xs uppercase tracking-[0.2em] text-gold hover:text-ink"
          >
            Continuar comprando
          </Link>
        </aside>
