...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col bg-ivory shadow-2xl transition-transform duration-400 ${
          ...
          <>
                ...
                {lines.map(({ product, qty }) => (
                  <li key={product.slug} className="flex gap-4 py-4">
                    <Link
                      ...
                      onClick={closeCart}
                      className="relative block h-24 w-20 shrink-0 overflow-hidden bg-white ring-1 ring-ink/5"
                    >
                      <ProductImage
                        src={product.image}
                        alt={product.name}
                    ...
                    <div className="flex flex-1 flex-col">
                      ...
                      <Link
                        href={`/produtos/${product.slug}`}
                        onClick={closeCart}
                        className="font-serif text-base leading-snug text-ink hover:text-gold"
                      >
                        {product.name}
                      </Link>
            ...
            <div className="border-t border-gold/30 px-6 py-5">
              <div className="mb-4 flex items-center justify-between">
                <span className="text-sm uppercase tracking-[0.2em] text-ink-soft">
                  Subtotal
                </span>
