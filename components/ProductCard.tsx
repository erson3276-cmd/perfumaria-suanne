...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group relative flex flex-col bg-ivory transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_24px_45px_-22px_rgba(25,19,16,0.3)]">
      {product.badge && (
      ...
      <div className="flex flex-1 flex-col border-t border-gold/15 px-4 pt-4 pb-5">
        ...
        <Link href={`/produtos/${product.slug}`}>
          <h3 className="mt-1.5 font-serif text-lg leading-snug text-ink transition-colors hover:text-gold">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto flex items-end justify-between gap-3 pt-4">
          <div>
            {hasDiscount && (
              <span className="block text-xs text-ink-soft/70 line-through">
          ...
          <button
            ...
            className={`border px-4 py-2.5 text-[10px] uppercase tracking-[0.22em] transition-all duration-300 ${
              product.inStock
                ...
                : "cursor-not-allowed border-ink-soft/30 text-ink-soft/40"
            }`}
          >
